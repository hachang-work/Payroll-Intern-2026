...
export const PIVOT_CACHE_VERSION = 8;
...
export function isMktRosterTypeColumn(typeRaw: string): boolean {
  ...
  return /^(LDEC|LDEM|LPAR|LRET|MOTH)/.test(type) || type === "MKT LOCAL";
}
    ...
    if (typeof localStorage !== "undefined") {
      ...
      if (cached) {
        ...
        if (
          ...
          // Validate that the cache contains only MKT Roster type columns
