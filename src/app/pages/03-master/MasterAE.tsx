...
export function MasterAE() {
  ...
  const currentTableSubtitle = useMemo(() => {
    switch (activeTab) {
      case "Sheet1_AE":
        return `Chi tiết Gross Pay kỳ ${currentPeriodVal} · ${currentData.data.length} dòng`;
      case "Deductions":
      ...
      default:
        return `Dữ liệu Master kỳ ${currentPeriodVal} · ${currentData.data.length} dòng`;
    }
      ...
      <AnimatePresence initial={false}>
        {view === "list" && (
          <motion.div
            ...
            className="flex-1 flex flex-col min-h-0 gap-4 relative overflow-hidden bg-transparent w-full p-1.5"
          >
